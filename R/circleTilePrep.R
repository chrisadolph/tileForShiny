...
function(trace) {
    ...
    if (is.null(trace$x)&&is.null(trace$top)) {
       stop("Circle trace contains no horizontal coordinates (both x and top are NULL)")
    }
    ...
    if (is.null(trace$y)&&is.null(trace$right)) {
        stop("Circle trace contains no vertical coordinates (both y and right are NULL)")
    }

    if (is.null(trace$r)) {
      stop("Circle trace contains no radius input")
    }
